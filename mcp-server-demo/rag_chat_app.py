...
async def process_query(query: str) -> str:
    ...
    try:
        response = azure_openai_client.chat_completions.create(
            engine=AZURE_OPENAI_CHAT_DEPLOYMENT_NAME,  # Use the GPT deployment name
            messages=messages,
            max_tokens=1000,
            temperature=0.7, 
        ...
        for tool in available_tools:
            if tool["name"] in assistant_response:
                tool_name = tool["name"]
